...
<div class="card border-white" style="width: 100rem;">
    <div class="card-body">
        <h5 style="text-align:center;">INPUT LAPORAN LABA RUGI <br/>{{ strtoupper(\App\Models\InformasiPokokBprPelapor::namaBpr($sandi_bpr)->nama_bpr) ?? '' }}</h5>

        <div style="height:30px" align="centre"></div>

        <form action="{{ url('simpan-laporan-laba-rugi') }}" method="POST">@csrf
            ...
            <table class="table table-stripped" id="table">
                ...
                <tr>
                    ...
                    <input type="hidden" name="pos[]" value="{{ $c->nama_coa  }}">

                    <?php $array = array(0,1,2,3,8,12,15,17,20,27,28,29,30,34, 40,44,45,47,50,55,56,61,70,74,75,76,83,89,90,92);?>

                    @if(in_array($key, $array))
                        <td style="font-size:11px"><input type="text" name="nominal[]" id="{{ $key+1 }}" style="text-align:right" class="form-control nominal" readonly></td>
                    @elseif($key == 90)
                        (<td style="font-size:11px"><input type="text" name="nominal[]" id="{{ $key+1 }}" style="text-align:right" class="form-control nominal" readonly></td>)
                    @else
                        <td style="font-size:11px"><input type="text" name="nominal[]" id="{{ $key+1 }}" style="text-align:right" class="form-control nominal"></td>
                    @endif
                </tr>
                @endforeach
...
$(document).ready(function(){
    ...
    $('#85, #86, #87, #88, #89').on('change keyup', function(){
        ...
        var laba_rugi_tahun_berjalan_sebelum_pajak = Number(laba_rugi_operasional) + Number(pendapatan_non_operasional) - Number(beban_non_operasional);
