...
<div class="card border-white" style="width: 100rem;">
    <div class="card-body">
        <div class="card border-white">
            <div class="card-body">
                <h5 style="text-align:center;">LAPORAN NERACA <br/>{{ strtoupper(\App\Models\InformasiPokokBprPelapor::namaBpr($sandi_bpr)->nama_bpr) ?? '' }}<br/>{{ strtoupper(bulan($bulan)) }} {{ $tahun }}</h5>
        ...
        <form action="{{ url('search-laporan-keuangan-bulanan') }}" method="POST">@csrf
            ...
            <div class="card-body">
                <form action="{{ url('update-laporan-keuangan-bulanan') }}" method="POST">@csrf
                    <table class="table table-hover">
                        ...
                        <tr>
                            <td>{{ $key+1 }}</td>
                            @php $array = array(3,6,13,28,32,35,38,47,48,57,64,65)@endphp
                            <td>{{ $d->pos }}</td>
                            <input type="hidden" name="id[]" value="{{ $d->id }}">
                            @if(in_array($key, $array))
                                <td><input type="text" class="form-control nominal" id="{{ $key+1 }}" name="nominal[]"  style="text-align:right" value="{{ number_format((float)$d->posisi_tanggal_laporan) }}" readonly></td>
                            @else
                                <td><input type="text" class="form-control nominal" id="{{ $key+1 }}" name="nominal[]"  style="text-align:right" value="{{ number_format((float)$d->posisi_tanggal_laporan) }}"></td>
                            @endif
                        </tr>
                        @endforeach
